...
> 审计基线：Aivulet 0.6.1（Build 8），2026-09-01。此文件是工程与提交准备材料，不是法律意见。最终答案必须以实际提交构建、最终翻译端点条款和 App Store Connect 当时的问卷为准。
...
当前不存在：开发者后端、独立 Aivulet 账号、分析、广告、追踪、崩溃上报、APNs 后台 Live Activity 远程更新
```
