...
Full conversation titles are excluded by default. Only after a separate sensitive-field confirmation may an explicitly identified title source be included after length, path, and secret-pattern filtering. The iPhone hides these titles whenever the app leaves the foreground and after every relaunch, and Live Activities never contain titles.
